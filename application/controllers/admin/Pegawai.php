...
class Pegawai extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('provider_model','fasilitas_model','login_model','user_model','gallery_model','transaksi_model','customer_model','pegawai_model'));
	}
	public function index(){
		...
		$data['content']='admin/pegawai_content';
		$data['slide']=null;
		$data['sidebar']='admin/sidebar_admin';
		$data['title']='Pegawai Apotek';

		//init data
		$data['provider'] = $this->pegawai_model->get()->result_array();
    ...
    function check_code(){
	 ...
	 public function post(){
        ...
        $data['user_login_id'] = $user_id;
            if($provider_id = $this->pegawai_model->add($data)){
                echo "1";
            }else{
                echo "0";
            }
        	...
        	$data_user_edit['email'] = $_POST['email'];
        	 $data['alamat'] = $_POST['alamat'];
            $data['nama'] = $_POST['nama'];
            $data['no_hp'] = $_POST['telp'];
            $data['email'] = $_POST['email'];
            ...
            $data['status'] = $_POST['status'];
            // $data['user_login_id'] = $user_id;
        	$this->user_model->update_user_login($_POST['user_login_id'],$data_user_edit);
            if($this->pegawai_model->update($_POST['id_pegawai'],$data)){
    ...
    public function get_by_id(){
        $id = $_POST['idx'];
        $result = $this->pegawai_model->get(array("id_pegawai"=>$id))->row_array();
        echo json_encode($result);
    }
